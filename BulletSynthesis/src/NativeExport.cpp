...
extern "C" {
	...
	EXPORT int Get_Physics_Num_Iterations() {
		return synthesis::PhysicsManager::GetInstance().GetNumIteration();
	}
	EXPORT void Set_Physics_Num_Iterations(int numIter) {
		synthesis::PhysicsManager::GetInstance().SetNumIteration(numIter);
	}
